...
function pintar(){
    ...
    $(".viajando").click(function(){
        console.log("clicked un dia viajando");
        var diaS = $(this).text();
        var parametroFecha = diaS+"-"+(fu.getMonth()+1)+"-"+fu.getFullYear();
        $.ajax({
            ...
            success: function( json ) {
                ...
                diaSeleccionado.setFullYear(fu.getFullYear());
                diaSeleccionado.setMonth(fu.getMonth());
                diaSeleccionado.setDate(diaS);
                console.log(json);
                cd(diaSeleccionado);
                $.each(json[0], function(i, item){
                  var $evento = $("<div>",{"class":"evento"});
                  var posicion = item.begin_date.split("T")[1].split(":");
                  var t = posicion[0]*44.7+posicion[1]*0.75;
                  $evento.css("position","absolute").css("z-index","1").css("width","100%").css("top",t+"px");
                  var posicion2 = item.end_date.split("T")[1].split(":");
                  var h = posicion2[0]*44.9+posicion2[1]*0.75;
                  h = h-t;
                  $evento.css("height",h+"px");
                  $evento.append($("<p>",{text:item.name}));
                  $evento.appendTo($("#horas"));
                });
            },
